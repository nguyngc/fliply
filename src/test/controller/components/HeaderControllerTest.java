...
import controller.AccountHelpController;
...
class AccountHelpControllerTest {
    ...
    void setUp() {
        ...
        injectHeaderField("titleLabel", title);
        injectHeaderField("subtitleLabel", subtitle);
        injectHeaderField("backButton", back);

        // Stub  method  AccountHelpController call
        Mockito.doAnswer(inv -> {
            title.setText(inv.getArgument(0));
            return null;
        }).when(header).setTitle(Mockito.anyString());

        Mockito.doAnswer(inv -> {
            subtitle.setText(inv.getArgument(0));
            return null;
        }).when(header).setSubtitle(Mockito.anyString());

        Mockito.doAnswer(inv -> {
            back.setVisible(inv.getArgument(0));
            return null;
        }).when(header).setBackVisible(Mockito.anyBoolean());
